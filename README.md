...
한국 증시의 **주도섹터**와 각 섹터의 **주도주**를 5분 단위 스냅샷으로 쌓아
...
> **거래시간(2026-09-14 KRX 매매거래시간 연장 시행)**: 프리마켓 07:00~07:50 신설,
> 정규장 09:00~15:30은 그대로, 애프터마켓이 16:00~20:00로 늘면서 정규장과 같은
> 실시간 접속매매로 바뀝니다(기존 10분 단일가). 이 프로젝트는 세 세션을
> 07~20시(사이사이 휴장)로 한 스케줄로 취급합니다.
> `MARKET_SESSIONS`(sector_matrix.py) 한 곳만 고치면 슬롯 라벨·수집 스케줄·장중 배지가 전부 따라갑니다.
...
| 영역 | 내용 |
|------|------|
| 미국장 마감 | S&P500·나스닥종합·나스닥100·필라델피아 반도체(SOX)·VIX·다우존스 종가·등락률 (다우 지수는 KIS API 미제공 → DIA ETF×100 으로 대체, 등락률은 지수와 거의 동일) |
| 미국 주도섹터 | SPDR 11개 GICS 섹터 ETF(XLK·XLF·XLE·XLV·XLY·XLP·XLI·XLB·XLU·XLRE·XLC) 등락률 순위 |
...
| 영역 | 내용 |
|------|------|
| 상단 카드 | 현재 가장 강한 주도섹터 4개 — 평균 등락률, 상승/하락 종목 수, 거래대금, 최근 5분 변화폭, 그리고 그 섹터의 **주도주** |
| 매트릭스 | 세로축 = 섹터, 가로축 = 5분 단위 시간대. 칸 색은 그 시점 섹터 평균 등락률 (한국식: 상승 빨강 / 하락 파랑, ±3%에서 최대 농도) |
| 섹터 행 클릭 | 구성 종목이 펼쳐지며 **종목 × 시간** 미니 히트맵 표시. 맨 위 ★ 종목이 현재 주도주 |
| 칸 클릭 | 하단 상세 패널에 해당 시각·해당 섹터의 종목 전체 순위(등락률·현재가·거래대금) |
| 정렬 | 현재 강도 / 최근 5분 모멘텀 / 거래대금 |

### ③ 미국장 모닝브리핑 탭

메인 대시보드 세 번째 탭. `docs/data/morning.json`(매일 06:00 KST 갱신)을 읽어
미국 지수·미국 주도섹터·미국 주요 종목 3개 표를 나란히 보여준다. 전일 한국 주도섹터까지
포함한 전체 브리핑은 `morning.html` 링크로.
...
| 순위표 | 1~30위. 직전 5분 대비 순위 변동(▲▼/NEW), 섹터 뱃지, 등락률, 현재가, 시가총액, 거래대금, 거래량, 거래량 증가율 |
| 기준 시각 | 5분 슬롯을 골라 그 시점의 순위표를 그대로 다시 볼 수 있음 |
| 종목 클릭 | 외국인/기관/프로그램 매수·매도·순매수 상세 + **캔들차트**(1/3/5/15분봉·일봉·주봉·월봉, 하단에 거래대금 바) — 휠/+−버튼으로 확대·축소, 드래그·하단 스크롤바로 좌우 이동 |
...
python sector_matrix.py --loop        # 장중 5분마다 자동 수집
...
**매일 오전 6시 55분(프리마켓 07:00 전)에 자동으로 시작**해서 장 마감 후 스스로 종료합니다. 주말이면 즉시 종료합니다.
