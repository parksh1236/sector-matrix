...
동작 요약
  ...
  2) 5분 단위 슬롯(0900, 0905, ... 1530, 1600, ... 2000)에 스냅샷으로 기록한다
     — 2026-09-14 한국거래소 매매거래시간 연장 시행: 정규장 09:00~15:30 은 그대로,
       애프터마켓이 16:00~20:00 로 늘고 기존 10분 단일가매매에서 정규장과 같은
       실시간 접속매매로 바뀜 (이 프로젝트는 정규장+애프터마켓을 09~20시 한 스케줄로 취급)
...
# 2026-09-14 KRX 매매거래시간 연장 시행: 프리마켓 07:00~07:50 신설, 정규장 09:00~15:30 은
# 그대로, 애프터마켓이 16:00~20:00 로 늘면서 실시간 접속매매로 바뀜(기존 10분 단일가).
# 세션의 시작·끝 시각을 여기 한 곳만 고치면 슬롯 라벨·수집 스케줄·프론트 "장중" 배지가 전부 따라감.
MARKET_SESSIONS = [(700, 750), (900, 1530), (1600, 2000)]
...
def is_market_time(now: datetime) -> bool:
    """평일이고 정규장(09:00~15:30) 또는 애프터마켓(16:00~20:00) 안인지 — 사이 휴장 시간대는 제외."""
    if now.weekday() >= 5:
        return False
    hm = now.hour * 100 + now.minute
...
def run_daily(push: bool) -> None:
    """
    하루 한 번 실행되는 모드 (launchd 가 매일 오전 6시 55분에 띄움 — 07시 프리마켓 전).
